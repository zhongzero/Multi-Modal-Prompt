...
def generate_clsss_images(class_prompt, class_data_dir, num_class_images, pretrained_model):
    
    if os.path.exists(f"{class_data_dir}/images"):
        os.system(f"rm -r {class_data_dir}/images")
    os.makedirs(f"{class_data_dir}/images", exist_ok=True)
    
    sample_batch_size=4
    pipe = StableDiffusionPipeline.from_pretrained(pretrained_model, safety_checker=None, dtype=torch.float16).to("cuda")
    bar = tqdm(range(math.ceil(num_class_images/sample_batch_size)))
    for i in bar:
        generate_num = min(sample_batch_size, num_class_images-i*sample_batch_size)
        bar.set_description(f"Generate {i*sample_batch_size+1}-th to {i*sample_batch_size+generate_num}-th class images")
        images = pipe(class_prompt, num_images_per_prompt=generate_num).images
        for j, image in enumerate(images):
            image.save(f"{class_data_dir}/images/{i*sample_batch_size+j}.jpg")

    with open(f"{class_data_dir}/caption.txt", "w") as f1, open(f"{class_data_dir}/urls.txt", "w") as f2, open(
        f"{class_data_dir}/images.txt", "w"
    ) as f3:
        for i in range(num_class_images):
            f1.write(f"{class_prompt}\n")
            f2.write("none\n")
            f3.write(f"{class_data_dir}/images/{i}.jpg" + "\n")
    return
...
def parse_args():
    ...
    parser.add_argument("--use_generate_data", help="generate data", action="store_true")
    parser.add_argument("--pretrained_model", help="pretrained model using in images generation", type=str)
    return parser.parse_args()


if __name__ == "__main__":
    args = parse_args()
    if args.use_generate_data:
        generate_clsss_images(args.class_prompt, args.class_data_dir, args.num_class_images, args.pretrained_model)
    # else:
